    ...
    Drawable(shader), m_pimpl(MakeScope<Impl>())
{
  ...
  auto layout = MakeRef<BufferLayout>(std::initializer_list<BufferElem>{
    BufferElem{ ShaderDataType::Float3, sizeof(float) * 3, 0, false },
    BufferElem{ ShaderDataType::Float4, sizeof(float) * 4, sizeof(float) * 3, false } });
  ...
  for (u32 i = 0; i < CIRCLE_POINTS; ++i)
  {
    ...
    const float circle_x = std::cos(angle) * radius + x;
    const float circle_y = std::sin(angle) * radius + y;
