...
f32 GE::Geom::AngleBetween(const GE::Vec3& a, const GE::Vec3& b)
{
  ...
  const f32 dotProduct = a.Dot(b);

  // Calculate the magnitudes of vectors a and b
  const f32 magA = a.Length();
  const f32 magB = b.Length();
  ...
  const f32 angleRadians = std::acos(cosineAngle);

  // Convert the angle from radians to degrees
  const f32 angleDegrees = Transform::Rad2Deg(angleRadians);
